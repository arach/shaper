...
import { traceFromImage } from './bezier-fit';
import type {
  BezierData,
  BezierSegment,
  NamedAnchor,
...
// ---------------------------------------------------------------------------
// Core context value — the shared editing engine
// ---------------------------------------------------------------------------
export interface ShaperCoreValue {
  // Canvas/Viewport
  zoom: number;
...
const ShaperCoreContext = createContext<ShaperCoreValue | null>(null);

export function useShaperCore() {
  const ctx = useContext(ShaperCoreContext);
  if (!ctx) throw new Error('useShaperCore must be used inside ShaperCoreProvider');
  return ctx;
}
...
// Provider props
// ---------------------------------------------------------------------------
interface ShaperCoreProviderProps {
  children: ReactNode;
  /** Base path for fetching demo data. Default '' (standalone), Hudson passes '/shaper'. */
  basePath?: string;
}

// ---------------------------------------------------------------------------
// Provider — the canonical editing engine
// ---------------------------------------------------------------------------
export function ShaperCoreProvider({ children, basePath = '' }: ShaperCoreProviderProps) {
  // ── Tools ──
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw) {
        ...
        if (session.dataUrl && session.image) {
          ...
          fetch(`${basePath}/talkie-bezier.json`).then((r) => r.json()).then((bezier) => {
    ...
    Promise.all([
      fetch(`${basePath}/talkie-bezier.json`).then((r) => r.json()),
      fetch(`${basePath}/talkie-anchors.json`).then((r) => r.json()),
    ]).then(([bezier, anchors]) => {
      setBezierData(bezier);
      setAnchorsData(anchors);
      historyRef.current = [JSON.stringify(bezier)];
  ...
  useEffect(() => {
    ...
    autoSaveRef.current = setTimeout(() => { saveToDisk(bezierData, smoothStates, projectId); }, 2000);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 't' || e.key === 'T') setAnimationModeEnabled(prev => !prev);
      if (animationModeEnabled) {
        if (e.key === ' ') { e.preventDefault(); setIsAnimating(prev => !prev); return; }
        if (e.key === 'r' || e.key === 'R') { e.preventDefault(); setIsAnimating(false); setAnimationProgress(0); return; }
      }
  ...
  // ── Trace ──
  const traceImageSrc = projectImage?.url || `${basePath}/talkie-silhouette.png`;
  const displayImageSrc = projectImage?.url || `${basePath}/talkie-original.png`;
  ...
  const { revealedHandles, revealedAnchors } = useMemo(() => {
    if (!bezierData || animationProgress === 0) return { revealedHandles: [] as ShaperCoreValue['revealedHandles'], revealedAnchors: [] as ShaperCoreValue['revealedAnchors'] };
    if (!showAnimationHandles && !showAnimationAngles) return { revealedHandles: [] as ShaperCoreValue['revealedHandles'], revealedAnchors: [] as ShaperCoreValue['revealedAnchors'] };
    ...
    const handles: ShaperCoreValue['revealedHandles'] = [];
    let segIdx = 0;
    for (const stroke of bezierData.strokes) {
      for (const seg of stroke) {
    ...
    const anchors: ShaperCoreValue['revealedAnchors'] = [];
    segIdx = 0;
    for (const stroke of bezierData.strokes) {
      for (const seg of stroke) {
        const isCurrent = segIdx === currentSegmentIndex;
  ...
  const anchorLabels = useMemo(() => {
    ...
    return anchorsData.anchors.map((anchor, i) => (
      <g key={i}>
        ...
        <text x={anchor.x + 12} y={anchor.y + 2} className="fill-white font-mono" fontSize={10}>{anchor.name}</text>
      </g>
    ));
  ...
  const strokeGroups = useMemo(() => {
    if (!anchorsData) return [] as [string, NamedAnchor[]][];
  ...
  return <ShaperCoreContext.Provider value={value}>{children}</ShaperCoreContext.Provider>;
}
